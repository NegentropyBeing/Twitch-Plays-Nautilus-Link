# Handles the actual network connection to Twitch or YouTube chat and
# parses incoming messages. In normal use you shouldn't need to touch this
# file -- import it and call twitch_connect()/twitch_receive_messages() (or
# the YouTube equivalents) from the main script instead.
...
class Twitch:
    ...
    def reconnect(self, delay):
        # Keeps retrying forever instead of raising on the first failed
        # attempt. This matters most after long idle periods, since that's
        # exactly when a sleeping machine, a Wi-Fi drop, or a brief network
        # blip is most likely to have broken the connection -- without this
        # loop, a single failed reconnect attempt would crash the whole
        ...
        if buffer:
            ...
            if not matches:
                ...
                if matches[0].start() != 0:
                    # The parser's first match doesn't start at the buffer's
                    # beginning, which means a message may have been missed
                    # or the buffer is unaligned with IRC message boundaries.
                    print('Warning: possible missed or misaligned IRC message during parsing.')
...
# Polls a YouTube livestream's chat by repeatedly fetching new messages
# using a continuation token, since YouTube's live chat API doesn't offer
# a persistent streaming connection the way Twitch's IRC does.
class YouTube:
    session = None
    config = {}
    payload = {}
